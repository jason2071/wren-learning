...
5. เก็บลง queries.yml (append)         แล้ว build + index
...
wren context build && wren memory index   # 5 เข้า memory
```

### ลำดับบังคับ (ห้ามสลับ — มี dependency)
- `context build` **ต้องก่อน** `memory index` (index อ่าน `target/mdl.json` ที่ build สร้าง)
- เพิ่ม column/relationship → `context build` **ต้องก่อน** รัน SQL ที่ใช้ field ใหม่
- `store` ไม่ขึ้นกับ build — เข้า memory ทันที (ต่างจาก `queries.yml` ที่ต้อง index)
...
> rule of thumb: **ตอบเฉยๆ = 1 คำสั่ง · เก็บถาวร = +store หรือ +queries.yml/build/index**
...
wren context build
wren memory index          # index โหลด queries.yml อัตโนมัติ
```
...
|---|---|---|
| field ครบแล้ว (เคสนี้) | `queries.yml` เท่านั้น | `context build` + `memory index` |
...
**กฎจำง่าย:** แตะ `models/`/`relationships.yml` → `context build`. แตะ `instructions.md`/`queries.yml` → `context build` + `memory index`

...
|---|---|---|
| `wren memory store --nl --sql` | **ไม่** (เข้า `.wren/` ที่ gitignore) | จดเร็วๆ ใช้ส่วนตัว ไม่อยากแชร์ |
| เพิ่มใน `queries.yml` + `memory load` | **ได้** (queries.yml อยู่ใน git) | อยากให้ทีม clone ไปแล้วได้ด้วย — **แนะนำ** |

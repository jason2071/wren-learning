...
| คำถาม | <span style="color:#cf3b25">raw SQL (ผิด)</span> | <span style="color:#247a4d">Wren (ถูก)</span> |
|---|---|---|
...
### The most important gotcha: queries.yml loads via `load`, not `index`

- `wren memory index` → สร้างแค่ auto browse queries ("List all products", "Total X in Y") — **ไม่ดึง**คู่ curate จาก queries.yml เข้า recall
- `wren memory load queries.yml` → import คู่ที่เราเขียนเข้า store **จริง** (อันนี้แหละที่ต้องใช้)
- `wren memory dump` → ทิศกลับ! export store เขียนทับ queries.yml (อย่ารันมั่ว)
...
| แก้อะไร | รันอะไร |
|---|---|
| `models/*.yml` หรือ `relationships.yml` | `wren context build` |
| `instructions.md` | `wren context build && wren memory index` |
| `queries.yml` | `wren memory load queries.yml` |
| เพิ่ม **model ใหม่** | `wren context build && wren memory index`<br>(+ `wren memory load queries.yml` ถ้าเพิ่มคู่ด้วย) |
...
|---|---|---|
| `wren memory store --nl --sql` | <span style="color:#cf3b25"><b>ไม่</b></span> (เข้า `.wren/` ที่ gitignore) | จดเร็วๆ ใช้ส่วนตัว ไม่อยากแชร์ |
| เพิ่มใน `queries.yml` + `memory load` | <span style="color:#247a4d"><b>ได้</b></span> (queries.yml อยู่ใน git) | อยากให้ทีม clone ไปแล้วได้ด้วย — **แนะนำ** |
